...
class TrapsManager {
    ...
    spawnTrap(type) {
        ...
        this.tablet.sendCommand(`spawn.trap ${type} ${x} ${y} ${z}`).then(() => {
    ...
    removeAll() {
        this.tablet.showConfirm('Remove ALL traps?', (confirmed) => {
            if (confirmed) {
                this.tablet.sendCommand('remove.traps all').then(() => {
    ...
    updateStats() {
        ...
        // For now, simulate via RCON
        this.tablet.sendCommand('trap.count').then(counts => {
            if (counts) {
                // Parse counts
                // Example: "shotgun:3,flame:1,sam:2,landmine:5,beartrap:2"
