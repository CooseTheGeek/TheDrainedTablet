// scheduler.js – DRAINED TABLET ULTIMATE v7.0.0
// Scheduled actions: create, edit, delete, and automatically execute timed commands.
...
class Scheduler {
    ...
    loadActions() {
        const saved = localStorage.getItem('tdl_scheduled_actions');
        return saved ? JSON.parse(saved) : [];
    }

    saveActions() {
        localStorage.setItem('tdl_scheduled_actions', JSON.stringify(this.actions));
    }
    ...
    attachEvents() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (e.target.classList.contains('run-action')) {
                const id = e.target.dataset.id;
                this.runAction(id);
            }
        });
    ...
    changeMonth(delta) {
        ...
        this.tablet.showToast('Month navigation not implemented', 'info');
    }
            ...
            html += `
                <div class="action-card ${action.active ? 'active' : 'paused'}">
                    ...
                    <div class="action-actions">
                        <button class="action-btn toggle-action" data-id="${action.id}">
                            ${action.active ? '⏸️ PAUSE' : '▶️ RESUME'}
                        </button>
                        <button class="action-btn delete-action" data-id="${action.id}">🗑️ DELETE</button>
                        <button class="action-btn run-action" data-id="${action.id}">⚡ RUN NOW</button>
                    </div>
                </div>
            `;
        });
    ...
    runAction(id) {
        ...
        if (action) {
            ...
            // Send RCON command
            ConnectionManager.executeCommand(action.fullCommand).then(() => {
                this.tablet.showToast('Action executed', 'success');
            }).catch(err => {
                this.tablet.showError('Execution failed: ' + err.message);
            });
        }
    }
    ...
    calculateNextRun(action) {
        ...
        return new Date(start.getTime() + (intervals + 1) * action.interval * 1000);
    }
        ...
        setInterval(() => {
            ...
            this.actions.forEach(action => {
                ...
                if (nextRun <= now) {
                    ...
                    if (action.count > 0) {
                        action.count--;
                        if (action.count === 0) action.active = false;
                    }
                }
            });
            this.saveActions();
    ...
    executeAction(action) {
        ...
        ConnectionManager.executeCommand(action.fullCommand).catch(err => {
