// inventoryViewer.js – DRAINED TABLET ULTIMATE v7.0.0
// Live inventory viewer for players. Displays belt, main, and wear slots with real data from RCON.
// All original features preserved, now with enhanced UI and auto‑refresh.
...
class InventoryViewer {
    ...
    createHTML() {
        const tab = document.getElementById('tab-inventoryViewer');
        if (!tab) return;
    ...
    async loadInventory(playerName) {
        this.tablet.showToast(`Loading inventory for ${playerName}...`, 'info');
        try {
            const response = await ConnectionManager.executeCommand(`player.inventory ${playerName}`);
            if (response) {
        ...
        lines.forEach(line => {
            ...
            items.forEach((itemStr, index) => {
                ...
                if (parts.length >= 3) {
                    ...
                    const itemData = { name, shortname, quantity, condition };
    ...
    createSlotHTML(item, section, index) {
        if (!item) {
            return `<div class="inventory-slot empty" data-section="${section}" data-index="${index}"><div class="slot-content">EMPTY</div></div>`;
        }
        return `
