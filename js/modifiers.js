// modifiers.js – DRAINED TABLET ULTIMATE v7.0.0
// Item and global modifiers: set collection, gather, loot, furnace, quarry, etc.
...
class Modifiers {
    ...
    loadModifiers() {
        const saved = localStorage.getItem('tdl_modifiers');
    ...
    loadGlobalModifiers() {
        const saved = localStorage.getItem('tdl_global_modifiers');
        return saved ? JSON.parse(saved) : {
            furnaceUsage: 1.0,
            furnaceOutput: 1.0,
            cookSpeed: 1.0,
    ...
    saveModifiers() {
        localStorage.setItem('tdl_modifiers', JSON.stringify(this.modifiers));
    }

    saveGlobalModifiers() {
        localStorage.setItem('tdl_global_modifiers', JSON.stringify(this.globalModifiers));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-modifiers');
        if (!tab) return;
    ...
    attachEvents() {
        ...
        // Delegate for item modifier actions
    ...
    renderItemModifiers() {
        ...
        // Sample items – in a full implementation, you'd use itemsDatabase
        const items = [
        ...
        let filtered = items;
        if (search) {
            filtered = filtered.filter(i => i.name.toLowerCase().includes(search) || i.shortname.includes(search));
        }
        if (category !== 'all') {
    ...
    clearAllModifiers() {
        if (!confirm('Clear ALL modifiers?')) return;
