...
class EconomySystem {
    ...
    renderBalances() {
        ...
        // Fetch real balances from server via RCON or local storage
        // For now, use mock data that will be replaced with RCON calls
        const players = [
    ...
    spinWheel() {
        ...
        // Simulate spin result (would be replaced with server-side RNG)
        ...
        // In production, send transaction to server via RCON
        this.tablet.sendCommand(`economy.adjust ${player} ${win ? '+' : '-'}${win ? winnings : bet}`).then(() => {
            this.addTransaction({
                player: player,
                type: 'spin',
            ...
            this.tablet.showError('Failed to record spin: ' + err.message);
        });
    ...
    giveFunds(player) {
        ...
        if (amount) {
            this.tablet.sendCommand(`economy.give ${player} ${amount}`).then(() => {
                this.addTransaction({
                    player: player,
                    type: 'give',
                ...
                this.tablet.showError('Failed to give funds: ' + err.message);
            });
        }
    }
    ...
    takeFunds(player) {
        ...
        if (amount) {
            this.tablet.sendCommand(`economy.take ${player} ${amount}`).then(() => {
                this.addTransaction({
                    player: player,
                    type: 'take',
                    amount: -parseInt(amount),
                ...
                this.tablet.showError('Failed to take funds: ' + err.message);
            });
        }
    }
    ...
    setFunds(player) {
        ...
        if (amount) {
            this.tablet.sendCommand(`economy.set ${player} ${amount}`).then(() => {
                this.addTransaction({
                    player: player,
                    type: 'set',
                    amount: parseInt(amount),
                ...
                this.tablet.showError('Failed to set funds: ' + err.message);
            });
        }
    }
