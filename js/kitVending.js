// kitVending.js – DRAINED TABLET ULTIMATE v7.0.0
// Kit vending machines: spawn, edit, assign kits, manage stock.
...
class KitVending {
    ...
    loadMachines() {
        const saved = localStorage.getItem('tdl_vending_machines');
    ...
    saveMachines() {
        localStorage.setItem('tdl_vending_machines', JSON.stringify(this.machines));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-kitVending');
        if (!tab) return;
    ...
    renderKitAssignments() {
        ...
        container.innerHTML = this.currentKitAssignments.map((assign, index) => {
    ...
    saveVM() {
        ...
        if (this.editingMachineId) {
            const index = this.machines.findIndex(m => m.id === this.editingMachineId);
            if (index !== -1) {
        ...
        this.renderMachines();
        document.getElementById('vm-modal').classList.add('hidden');
        this.tablet.showToast(`Vending machine ${name} saved`, 'success');
    }
    ...
    restockVM(id) {
        const machine = this.machines.find(m => m.id === id);
        if (machine) {
            machine.kits.forEach(kit => { kit.stock = 10; });
...
document.addEventListener('DOMContentLoaded', () => {
    window.kitVending = new KitVending();
});