// zorp.js – DRAINED TABLET ULTIMATE v7.0.0
// ZORP zone management – special zones with ownership and PvP flags.
...
class ZorpZones {
    ...
    createHTML() {
        const tab = document.getElementById('tab-zorp');
        if (!tab) return;
                ...
                <div id="zorp-modal" class="modal hidden">
                    <div class="modal-content zorp-modal">
                        ...
                        <div class="checkbox-group">
                            <label><input type="checkbox" id="zorp-nopvp"> No PvP</label>
                            <label><input type="checkbox" id="zorp-nobuild"> No Building</label>
                            <label><input type="checkbox" id="zorp-nonpc"> No NPC Damage</label>
                            <label><input type="checkbox" id="zorp-noheal"> No Healing</label>
                            <label><input type="checkbox" id="zorp-noloot"> No Loot</label>
                            <label><input type="checkbox" id="zorp-safe"> Safe Zone</label>
                            <label><input type="checkbox" id="zorp-radiation"> Radiation Zone</label>
                            <label><input type="checkbox" id="zorp-kill"> Kill Zone</label>
                        </div>
                        
                        <div class="form-group" id="radiation-group" style="display: none;">
                            <label>Radiation Amount: <span id="zorp-radiation-value">10</span></label>
    ...
    async loadZones() {
        // For now, use local storage or bridge – we'll implement bridge later
        const saved = localStorage.getItem('tdl_zorp_zones');
        this.zones = saved ? JSON.parse(saved) : [];
        this.renderZones();
    }

    saveZones() {
        localStorage.setItem('tdl_zorp_zones', JSON.stringify(this.zones));
    }
    ...
    deleteZone(id) {
        if (!confirm('Delete this ZORP zone?')) return;
...
document.addEventListener('DOMContentLoaded', () => {
    window.zorpZones = new ZorpZones();
});