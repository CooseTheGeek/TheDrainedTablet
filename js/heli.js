// heli.js – DRAINED TABLET ULTIMATE v7.0.0
// Helicopter controls: spawn/despawn patrol heli, chinook, scrap heli, minicopter.
...
class Heli {
    ...
    loadSettings() {
        const saved = localStorage.getItem('tdl_heli_settings');
    ...
    saveSettings() {
        localStorage.setItem('tdl_heli_settings', JSON.stringify(this.settings));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-heli');
        if (!tab) return;
    ...
    spawnPatrol() {
        ...
        ConnectionManager.executeCommand('spawn.heli').then(() => {
    ...
    despawnPatrol() {
        this.active.patrolHeli = false;
        document.getElementById('patrol-status').innerText = '⚫ INACTIVE';
        ConnectionManager.executeCommand('despawn.heli').then(() => {
    ...
    spawnChinook() {
        ...
        ConnectionManager.executeCommand('spawn.chinook').then(() => {
    ...
    despawnChinook() {
        this.active.chinook = false;
        document.getElementById('chinook-active').innerText = '⚫ INACTIVE';
        ConnectionManager.executeCommand('despawn.chinook').then(() => {
    ...
    forceDrop() {
        ConnectionManager.executeCommand('chinook.forcedrop').then(() => {
    ...
    spawnScrapHeli() {
        ConnectionManager.executeCommand('spawn.scrapheli').then(() => {
    ...
    spawnMini() {
        ConnectionManager.executeCommand('spawn.minicopter').then(() => {
    ...
    spawnAtLocation(x, z) {
        this.tablet.showToast(`Spawning at (${x}, 0, ${z}) – not implemented`, 'info');
    }
    ...
    resetSettings() {
        if (!confirm('Reset helicopter settings?')) return;
        this.settings = {
            bulletAccuracy: 50,
            bulletDamageScale: 1.0,
            aggroRange: 100,
