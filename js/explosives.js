// explosives.js – DRAINED TABLET ULTIMATE v7.0.0
// Explosives and raid balancing settings.
...
class Explosives {
    ...
    loadSettings() {
        const saved = localStorage.getItem('tdl_explosives_settings');
    ...
    saveSettings() {
        localStorage.setItem('tdl_explosives_settings', JSON.stringify(this.settings));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-explosives');
        if (!tab) return;
    ...
    resetSettings() {
        if (!confirm('Reset explosives settings?')) return;
        this.settings = {
            c4Damage: 1.0,
            rocketDamage: 1.0,
            satchelDamage: 1.0,
...
document.addEventListener('DOMContentLoaded', () => {
    window.explosives = new Explosives();
});