...
class ServerStatus {
    ...
    updateStats() {
        if (!this.tablet.connected) {
            // Use mock data when disconnected – but this is only for display, not for server commands
            this.stats = {
                players: 12,
                maxPlayers: 100,
                queue: 0,
    ...
    handleQuickAction(action) {
        ...
        switch(action) {
            case 'restart':
                this.tablet.showConfirm('Restart server? This will kick all players.', (confirmed) => {
                    if (confirmed) {
                        ...
                        this.tablet.sendCommand('global.restart');
                    }
                });
                break;
            case 'save':
                this.tablet.showToast('Saving world...', 'info');
                this.tablet.sendCommand('server.save').then(() => {
            ...
            case 'broadcast':
                ...
                if (msg) {
                    this.tablet.sendCommand(`broadcast ${msg}`);
            ...
            case 'announce':
                ...
                if (announce) {
                    this.tablet.sendCommand(`say ${announce}`);
            ...
            case 'wipe':
                this.tablet.showConfirm('⚠️ WIPE SERVER? ⚠️\nThis will erase everything!', (confirmed) => {
                    if (confirmed) {
                        ...
                        // Actual wipe command would depend on server type
                    }
                });
                break;
        }
