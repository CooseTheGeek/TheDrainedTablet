...
// Live map with real player positions, map image (if available), and grid fallback.
// All original features preserved, now with enhanced zoom/pan and performance.
...
class Livemap {
    ...
    init() {
        ...
        window.addEventListener('players-updated', (e) => {
            ...
            if (this.mapLoaded || this.useFallback) this.drawMap();
        });
        window.addEventListener('tab-changed', (e) => {
            if (e.detail.tab === 'livemap') this.refresh();
        });
        window.addEventListener('map-refresh', () => this.loadMap());
    }
    ...
    loadMap() {
        ...
        this.mapImage.onerror = () => {
            ...
            // Try HTTP as last resort (will be blocked by mixed content, but fallback already active)
    ...
    trackMouse(e) {
        ...
        const coordsEl = document.querySelector('.map-coords');
        if (coordsEl) coordsEl.innerHTML = `X: ${gameX}  Z: ${gameZ}`;
    }
