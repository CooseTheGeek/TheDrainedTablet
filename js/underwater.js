...
class UnderwaterManager {
    ...
    spawnSub() {
        this.tablet.sendCommand('spawn.submarine').then(() => {
    ...
    resetLab() {
        this.tablet.showConfirm('Reset underwater laboratory?', (confirmed) => {
            if (confirmed) {
                this.tablet.sendCommand('reset.underwaterlab').then(() => {
    ...
    spawnCrate() {
        this.tablet.sendCommand('spawn.lockedcrate').then(() => {
    ...
    gotoLocation(x, z) {
        this.tablet.sendCommand(`teleport.topos ${x} 0 ${z}`).then(() => {
    ...
    updateStats() {
        // Fetch from server
        this.tablet.sendCommand('underwater.stats').then(stats => {
            if (stats) {
            ...
            document.getElementById('stat-visibility').innerText = '15m';
        });
    }
