...
class VoiceCommands {
    ...
    getOnlinePlayers() {
        const count = this.tablet.realPlayers.length;
    ...
    spawnCrate() {
        this.tablet.sendCommand('spawn.crate').then(() => {
    ...
    getTime() {
        this.tablet.sendCommand('env.time').then((time) => {
    ...
    getWeather() {
        this.tablet.sendCommand('env.weather').then((weather) => {
    ...
    mutePlayer(player) {
        this.tablet.sendCommand(`mute ${player} 30`).then(() => {
    ...
    giveKit(kit, player) {
        this.tablet.sendCommand(`kit.give ${player} ${kit}`).then(() => {
