// voiceCommands.js – DRAINED TABLET ULTIMATE v7.0.0
// Voice command interface: speech recognition, command execution, wake word, and feedback.
// All original features preserved, now integrated with RCON and enhanced UI.
...
class VoiceCommands {
            ...
            <div class="voice-container">
                ...
                <div class="voice-settings">
                    ...
                    <div class="checkbox-item">
                        <label><input type="checkbox" id="voice-feedback" checked> Voice Feedback</label>
                    </div>
                    
                    <div class="checkbox-item">
                        <label><input type="checkbox" id="require-wake" checked> Require Wake Word</label>
                    </div>
                </div>
    ...
    processCommand(transcript) {
        ...
        let wakeWord = this.wakeWord;
        if (document.getElementById('wake-word')?.value === 'custom') {
            wakeWord = document.getElementById('custom-wake-word')?.value.toLowerCase() || this.wakeWord;
        } else {
            wakeWord = document.getElementById('wake-word')?.value || this.wakeWord;
        }
    ...
    async banPlayer(player) {
        ...
        try {
            await ConnectionManager.executeCommand(`ban ${player}`);
    ...
    async spawnHeli() {
        try {
            await ConnectionManager.executeCommand('spawn.heli');
    ...
    async spawnCrate() {
        try {
            await ConnectionManager.executeCommand('spawn.crate');
    ...
    async getTime() {
        try {
            ...
            this.tablet.showError(`Failed to get time: ${err.message}`);
        }
    }
    ...
    async getWeather() {
        try {
            ...
            this.tablet.showError(`Failed to get weather: ${err.message}`);
        }
    }

    async mutePlayer(player) {
        try {
            await ConnectionManager.executeCommand(`mute ${player} 30`);
            this.speak(`${player} has been muted for 30 minutes`);
    ...
    async giveKit(kit, player) {
        try {
            await ConnectionManager.executeCommand(`kit.give ${player} ${kit}`);
            this.speak(`Giving ${kit} kit to ${player}`);
            this.addCommandHistory('giveKit', `Gave ${kit} to ${player}`);
        } catch (err) {
            this.tablet.showError(`Give kit failed: ${err.message}`);
        }
    }
