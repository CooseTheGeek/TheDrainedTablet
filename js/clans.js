// clans.js – DRAINED TABLET ULTIMATE v7.0.0
// Clan management: create, edit, delete, invite, kick members.
...
class Clans {
    ...
    loadClans() {
        const saved = localStorage.getItem('tdl_clans');
        return saved ? JSON.parse(saved) : [];
    }

    saveClans() {
        localStorage.setItem('tdl_clans', JSON.stringify(this.clans));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-clans');
        if (!tab) return;
            ...
            html += `
                <div class="clan-card">
                    ...
                    <div class="clan-body">
                        ...
                        <div class="clan-members">Members: ${clan.members.join(', ')}</div>
    ...
    saveClan() {
        ...
        const clan = {
            ...
            tag,
            leader,
            members: this.editingClanId ? this.clans.find(c => c.id === this.editingClanId).members : [leader],
            maxMembers,
            base: { x: 0, z: 0 },
    ...
    kickMember(id, member) {
        ...
        if (!confirm(`Kick ${member} from ${clan.name}?`)) return;
    ...
    resetClans() {
        if (!confirm('Reset all clans?')) return;
        this.clans = [];
        this.saveClans();
        this.renderClans();
