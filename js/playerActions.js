// playerActions.js – DRAINED TABLET ULTIMATE v7.0.0
// Player actions: warnings, mutes, kicks, bans, votekick/voteban, with real RCON execution.
// All original features preserved, now with enhanced UI and persistent warnings/mutes.
...
class PlayerActions {
    ...
    createHTML() {
        const tab = document.getElementById('tab-playerActions');
        if (!tab) return;
    ...
    executeCommand(cmd, player) {
        switch(cmd) {
            ...
            case 'kick':
                ...
                if (kickReason) {
                    ConnectionManager.executeCommand(`kick ${player} "${kickReason}"`).then(() => {
            ...
            case 'ban':
                ...
                if (banReason) {
                    ...
                    ConnectionManager.executeCommand(`ban ${player} "${banReason}"`).then(() => {
    ...
    clearMutes() {
        if (!confirm('Clear ALL mutes?')) return;
...
document.addEventListener('DOMContentLoaded', () => {
    window.playerActions = new PlayerActions();
});