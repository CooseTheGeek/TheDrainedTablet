// drained-bases.js – DRAINED TABLET v7.0.0 (Admin editable base blueprints)
...
class DrainedBases {
    ...
    getDefaultBlueprints() {
        ...
        return [
            { id: 1, name: "2x1 Starter", description: "2x1 with TC, sleeping bag, key lock, and sheet metal door.", price: 350,
              ...
              items: [{ shortname: "cupboard.tool", amount: 1 }, { shortname: "sleepingbag", amount: 1 }, { shortname: "lock.key", amount: 1 }, { shortname: "door.hinged.metal", amount: 1 }],
              image: "https://www.corrosionhour.com/img/items/wood.png" },
            { id: 2, name: "2x2 Medium", description: "2x2 with TC, sleeping bag, lock, metal door.", price: 600,
              materials: { wood: 600, stone: 900, metal: 200, hqm: 0 },
              items: [{ shortname: "cupboard.tool", amount: 1 }, { shortname: "sleepingbag", amount: 1 }, { shortname: "lock.key", amount: 1 }, { shortname: "door.hinged.metal", amount: 1 }],
              image: "https://www.corrosionhour.com/img/items/metal.fragments.png" },
            { id: 3, name: "3x3 Fortress", description: "Large 3x3 base with TC, bag, lock, metal door.", price: 1200,
              materials: { wood: 1200, stone: 1800, metal: 800, hqm: 50 },
              items: [{ shortname: "cupboard.tool", amount: 1 }, { shortname: "sleepingbag", amount: 1 }, { shortname: "lock.key", amount: 1 }, { shortname: "door.hinged.metal", amount: 1 }],
    ...
    async loadMyPurchases() {
        ...
        if (!playerId && !this.access.isMasterUser()) { this.purchases = []; return; }
        const saved = localStorage.getItem('tdl_master_purchases');
        ...
        tab.innerHTML = `
            <div class="bps-container">
                <div class="bps-header">
                    ...
                    ${isAdmin ? '<button id="add-blueprint" class="bps-btn primary">+ ADD BASE</button>' : ''}
                </div>
            ...
            <div id="bp-modal" class="modal hidden">
                <div class="modal-content">
                    <h3 id="bp-modal-title">Add/Edit Base</h3>
                    ...
                    <h4>Materials</h4>
                    <div class="form-row"><label>Wood</label><input type="number" id="bp-wood"><label>Stone</label><input type="number" id="bp-stone"><label>Metal</label><input type="number" id="bp-metal"><label>HQM</label><input type="number" id="bp-hqm"></div>
                    <h4>Extra Items</h4>
                    <div id="bp-items-list"></div>
                    <button id="add-bp-item" class="small-btn">+ Add Item</button>
    ...
    addBlueprintItemField() {
        const container = document.getElementById('bp-items-list');
        const idx = container.children.length;
        const div = document.createElement('div');
        div.className = 'form-row';
        div.innerHTML = `<input type="text" placeholder="shortname" id="bp-item-short-${idx}" value=""><input type="number" placeholder="amount" id="bp-item-amt-${idx}" value="1"><button class="small-btn remove-bp-item" data-idx="${idx}">✕</button>`;
        container.appendChild(div);
        div.querySelector('.remove-bp-item').addEventListener('click', () => div.remove());
    }
            ...
            if (bp) {
                ...
                if (bp.items) {
                    bp.items.forEach((item, i) => {
                        const div = document.createElement('div');
                        div.className = 'form-row';
                        div.innerHTML = `<input type="text" placeholder="shortname" id="bp-item-short-${i}" value="${item.shortname}"><input type="number" placeholder="amount" id="bp-item-amt-${i}" value="${item.amount}"><button class="small-btn remove-bp-item" data-idx="${i}">✕</button>`;
                        document.getElementById('bp-items-list').appendChild(div);
                        div.querySelector('.remove-bp-item').addEventListener('click', () => div.remove());
                    });
                }
            }
    ...
    async fetchBalance() {
        const playerId = AppState.user?.platformId;
        if (!playerId && !this.access.isMasterUser()) { document.getElementById('bps-player-balance').innerText = '?'; return; }
        try {
            const bal = await ConnectionManager.executeCommand(`economy.balance "${playerId}"`);
            document.getElementById('bps-player-balance').innerText = parseInt(bal) || 0;
        } catch(e) { document.getElementById('bps-player-balance').innerText = '?'; }
    }
