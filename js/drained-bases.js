...
class DrainedBases {
    ...
    async loadMyPurchases() {
        ...
        if (!playerId) {
            if (this.access.isMasterUser()) {
                ...
                this.purchases = mp ? JSON.parse(mp) : [];
                return;
            }
            this.purchases = [];
            return;
    ...
    saveMasterPurchases() {
        if (this.access.isMasterUser()) localStorage.setItem('tdl_master_purchases', JSON.stringify(this.purchases));
    }
    ...
    renderOwned() {
        ...
        if (owned.length === 0) { container.innerHTML = '<div class="no-owned">No bases ready to claim</div>'; return; }
    ...
    refresh() {
        ...
        this.renderOwned();
        this.fetchBalance();
        toast.success('Base blueprints refreshed');
    }
}
