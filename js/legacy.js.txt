// LEGACYSTUDIO CLANS - DRAINED TABLET ULTIMATE v7.0.0
...
class LegacyClans {
    ...
    loadClans() {
        const saved = localStorage.getItem('drained_legacy_clans');
    ...
    saveClans() {
        localStorage.setItem('drained_legacy_clans', JSON.stringify(this.clans));
    }

    init() {
        this.createLegacyHTML();
        this.setupEventListeners();
        
        window.addEventListener('tab-changed', (e) => {
            if (e.detail.tab === 'legacy') {
                this.refresh();
            }
        });
    }
        ...
        legacyTab.innerHTML = `
            <div class="legacy-container">
                <div class="legacy-header">
                    <h2>👥 LEGACYSTUDIO CLANS</h2>
                    <button id="create-clan" class="legacy-btn primary">➕ CREATE CLAN</button>
                </div>

                <div class="clans-grid" id="clans-grid"></div>

                <div class="clan-actions">
                    <button id="refresh-clans" class="legacy-btn">🔄 REFRESH</button>
                    <button id="reset-clans" class="legacy-btn">🔄 RESET</button>
                </div>
                ...
                <div id="clan-modal" class="modal hidden">
                    <div class="modal-content clan-modal">
                        ...
                        <div class="modal-actions">
                            <button id="save-clan" class="legacy-btn primary">SAVE</button>
                            <button id="cancel-clan" class="legacy-btn">CANCEL</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    refresh() {
        this.renderClans();
        this.tablet.showToast('Legacy clans refreshed', 'success');
    }
}

// Initialize when tablet is ready
document.addEventListener('DOMContentLoaded', () => {
    window.legacyClans = new LegacyClans(window.drainedTablet);
});