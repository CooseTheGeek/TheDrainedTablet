...
class ChatSystem {
    ...
    startAutoRefresh() {
        ...
        this.refreshInterval = setInterval(() => {
            if (this.autoRefresh && this.tablet.connected) {
                // Simulate new messages
                const mockMessages = [
                    { user: 'RustGod', msg: 'Anyone want to raid?' },
                    { user: 'BuilderBob', msg: 'Building at outpost' },
                    { user: 'PvPKing', msg: 'Need guns!' }
                ];
                
                const random = mockMessages[Math.floor(Math.random() * mockMessages.length)];
                this.addMessage(random.user, random.msg);
            }
        }, 30000);
    }
    ...
    updatePlayerList() {
        ...
        // Get online players from tablet or use mock data
        const players = ['RustGod', 'BuilderBob', 'PvPKing', 'RaiderSue'];
    ...
    mutePlayer() {
        ...
        if (!this.mutedPlayers.includes(player)) {
            ...
            this.saveMuted();
            this.renderMessages();
            this.tablet.showToast(`${player} muted`, 'warning');
            this.addMessage('SERVER', `${player} was muted`, 'server');
        }
    }
    ...
    banPlayer() {
        ...
        this.tablet.showConfirm(`Ban ${player} for ${duration}h?`, (confirmed) => {
            if (confirmed) {
