...
class SecurityDoor {
    ...
    setupEventListeners() {
        ...
        // Quick admin override (Ctrl+M) for testing
    ...
    tryUnlock() {
        ...
        // Primary masters
        if (this.currentCode === this.tablet.accessCode) {
            valid = true;
            user = 'CooseTheGeek';
        } else if (this.currentCode === this.tablet.backupCode) {
            valid = true;
            user = 'CooseTheGeek';
        } else {
            // Check other users from tablet's user list
    ...
    unlockDoor(user) {
        ...
        this.tablet.userLevel = this.tablet.loadUsers()[user]?.level || 'user';
        document.getElementById('profile-link').innerText = `👤 ${user}`;
