// halloween.js – DRAINED TABLET ULTIMATE v7.0.0
// Halloween event manager: scarecrows, murderers, candy, and seasonal settings.
...
class Halloween {
    ...
    loadSettings() {
        const saved = localStorage.getItem('tdl_halloween_settings');
    ...
    saveSettings() {
        localStorage.setItem('tdl_halloween_settings', JSON.stringify(this.settings));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-halloween');
        if (!tab) return;
                ...
                <div class="halloween-grid">
                    <div class="halloween-section">
                        ...
                        <div class="checkbox-item">
                            <label><input type="checkbox" id="scarecrow-throw" ${this.settings.scarecrowsThrowBeancans ? 'checked' : ''}> Scarecrows throw beancans</label>
                        </div>
    ...
    spawnScarecrow() {
        if (!confirm('Spawn a scarecrow at your location?')) return;
    ...
    removeAllScarecrows() {
        if (!confirm('Remove all scarecrows?')) return;
        this.scarecrows = [];
        this.updateStats();
    ...
    spawnMurderer() {
        if (!confirm('Spawn a murderer at your location?')) return;
        this.tablet.showToast('Murderer spawned', 'success');
        this.updateStats();
    }

    removeAllMurderers() {
        if (!confirm('Remove all murderers?')) return;
    ...
    resetSettings() {
        if (!confirm('Reset Halloween settings to default?')) return;
        this.settings = {
            scarecrowHealth: 500,
            scarecrowDamage: 50,
            scarecrowAggroRange: 30,
...
document.addEventListener('DOMContentLoaded', () => {
    window.halloween = new Halloween();
});