// tradingFloor.js – DRAINED TABLET ULTIMATE v7.0.0
// Player trading system: listings, offers, market prices, and trade history.
// All original features preserved, now integrated with bridge and enhanced UI.
...
class TradingFloor {
    ...
    loadListings() {
        const saved = localStorage.getItem('tdl_trading_listings');
        return saved ? JSON.parse(saved) : [];
    }

    loadTrades() {
        const saved = localStorage.getItem('tdl_trading_trades');
        return saved ? JSON.parse(saved) : [];
    }

    loadPrices() {
        const saved = localStorage.getItem('tdl_trading_prices');
        return saved ? JSON.parse(saved) : {
            'rifle.ak': 450,
            'explosive.timed': 800,
    ...
    loadHistory() {
        const saved = localStorage.getItem('tdl_market_history');
        return saved ? JSON.parse(saved) : [];
    }

    saveAll() {
        localStorage.setItem('tdl_trading_listings', JSON.stringify(this.listings));
        localStorage.setItem('tdl_trading_trades', JSON.stringify(this.trades));
        localStorage.setItem('tdl_trading_prices', JSON.stringify(this.prices));
        localStorage.setItem('tdl_market_history', JSON.stringify(this.marketHistory));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-trading');
        if (!tab) return;
    ...
    renderListings() {
            ...
            html += `
                <div class="listing-card">
                    ...
                    <div class="listing-actions">
                        <button class="trade-btn buy-listing" data-id="${listing.id}">BUY</button>
                        <button class="trade-btn message-seller" data-id="${listing.id}">💬 MESSAGE</button>
                    </div>
                </div>
            `;
        });
        ...
        container.querySelectorAll('.buy-listing').forEach(btn => {
            btn.addEventListener('click', () => this.buyListing(btn.dataset.id));
        });
        container.querySelectorAll('.message-seller').forEach(btn => {
            btn.addEventListener('click', () => this.messageSeller(btn.dataset.id));
        });
    }
    ...
    renderMyTrades(tab = 'active') {
                ...
                <div class="trade-card ${trade.status}">
                    ...
                    ${trade.status === 'active' ? `
                        <div class="trade-actions">
                            <button class="trade-btn accept-trade" data-id="${trade.id}">✓ ACCEPT</button>
                            <button class="trade-btn decline-trade" data-id="${trade.id}">✗ DECLINE</button>
                        </div>
                    ` : ''}
                </div>
            `;
        ...
        container.querySelectorAll('.accept-trade').forEach(btn => {
            btn.addEventListener('click', () => this.acceptTrade(btn.dataset.id));
        });
        container.querySelectorAll('.decline-trade').forEach(btn => {
            btn.addEventListener('click', () => this.declineTrade(btn.dataset.id));
        });
    }
        ...
        modal.innerHTML = `
            <div class="trading-modal-content">
                ...
                <div class="modal-actions">
                    <button class="trading-btn create-listing-confirm">CREATE</button>
                    <button class="trading-btn cancel">CANCEL</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
        modal.querySelector('.create-listing-confirm').addEventListener('click', () => {
            this.createListing(modal);
        });
    ...
    createListing(modal) {
        const type = modal.querySelector('#listing-type').value;
        const item = modal.querySelector('#listing-item').value;
        const quantity = parseInt(modal.querySelector('#listing-quantity').value);
        const price = parseInt(modal.querySelector('#listing-price').value);
        const notes = modal.querySelector('#listing-notes').value;
        ...
        const listing = {
            ...
            seller: AppState.user.username || 'Anonymous',
            type,
            item,
            quantity,
            price,
    ...
    buyListing(listingId) {
        ...
        if (!listing) return;
        if (!confirm(`Buy ${listing.quantity}x ${listing.item} for ${listing.price} scrap?`)) return;
        this.listings = this.listings.filter(l => l.id !== listingId);
        this.marketHistory.unshift({
            item: listing.item,
            quantity: listing.quantity,
            price: listing.price,
            buyer: AppState.user.username,
...
document.addEventListener('DOMContentLoaded', () => {
    window.tradingFloor = new TradingFloor();
});