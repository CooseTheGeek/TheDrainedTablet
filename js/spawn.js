// spawn.js – DRAINED TABLET ULTIMATE v7.0.0
// Spawn manager: spawn crates, keycards, barrels, nodes, vehicles, structures.
...
class Spawn {
    ...
    loadHistory() {
        const saved = localStorage.getItem('tdl_spawn_history');
        return saved ? JSON.parse(saved) : [];
    }

    saveHistory() {
        localStorage.setItem('tdl_spawn_history', JSON.stringify(this.spawnHistory.slice(0, 100)));
    }
    ...
    createHTML() {
        const tab = document.getElementById('tab-spawn');
        if (!tab) return;
    ...
    async spawnItem(shortname, name) {
        ...
        try {
            await ConnectionManager.executeCommand(`spawn.entity ${shortname} ${x} ${y} ${z}`);
            this.spawnHistory.unshift({
                item: name,
                shortname,
                position: { x, y, z },
                time: new Date().toISOString(),
                spawnedBy: AppState.user.username
            });
            this.saveHistory();
            this.renderHistory();
            this.tablet.showToast(`Spawned ${name} at (${x}, ${y}, ${z})`, 'success');
        } catch (err) {
            this.tablet.showError(`Failed to spawn: ${err.message}`);
        }
    }
    ...
    clearHistory() {
        if (!confirm('Clear spawn history?')) return;
        this.spawnHistory = [];
        this.saveHistory();
        this.renderHistory();
